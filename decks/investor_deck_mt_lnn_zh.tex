...
\begin{frame}{双产品线：认知慢思维 + 边缘快思维}
\small
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{M 系列 (M1/M2) —— 认知慢思维}
...
\begin{itemize}
    ...
    \item \textbf{快权重跨会话记忆：} 内置快权重矩阵把关键 key$\to$value 绑定写入 $O(1)$ 状态，在整个上下文窗口被丢弃后仍可召回（0.56，注意力/LoRA 结构性为 0）；状态落盘快照跨进程恢复\textbf{逐位无损}。
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep=0.3em
    ...
    \item \textbf{1232 项自动测试常驻绿灯}、CPU 全过，每条能力主张都有脚本和产物背书。
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\end{columns}
\vspace{0.3em}
{\footnotesize\itshape 诚实边界：EWC、经验回放与生成式做梦重放均已在小规模上多种子验证，\textbf{尚未在 $\geq$1B 规模复现}。}
\end{frame}
...
\begin{frame}{核心验证 2：上下文窗口被丢弃后仍然记得}
\begin{itemize}
...
\end{itemize}
\vspace{0.2em}
\textbf{来自文档问答适配器}（独立产品线 —— 围绕\emph{冻结}基座模型的检索/压缩层，不是液态架构）：
\begin{itemize}\itemsep=0.4em
    \item \textbf{金融文档：} 多域标注评测集 \textbf{91.7\%（44/48）}，每题仅约 \textbf{1.3 次模型调用、约 2.8k tokens}，全程本地私有化。179 项测试全绿。
\end{itemize}
\vspace{0.2em}
{\footnotesize 规划中（情景，非承诺）：车载座舱系统、端侧个人助手、工业哨兵机器人（DualSpeedSentry 服务栈已容器化）。长期目标是让两条线\emph{合流} —— 用架构原生的液态记忆替换适配器的词法检索层。}
\vspace{0.3em}
...
\begin{itemize}\itemsep=0.4em
    ...
    \item \textbf{长线体验（跨窗口/跨会话记忆）：} 类似 Mamba 的传统线性模型没有可持久化的联想记忆。AwareLiquid 的快权重状态在\textbf{整个上下文窗口被丢弃后}仍能召回关键绑定（0.56 vs 注意力/LoRA 的 0.000），且状态快照跨进程恢复逐位无损 —— 长期陪伴式体验的架构基础。
...
\begin{itemize}
    ...
    \item \textbf{对比现有的高效线性架构（如 Mamba / RWKV 等）：}
        此类架构面临着拉长文本即产生“记忆退化衰减”的困境，且没有可跨会话持久化的联想记忆。MT-LNN 借助门控与动态路由调度，从底层机制上克服了长期维持多向特征的衰减问题。
    \item \textbf{对比 Liquid AI（LFM 系列）：}
        Liquid AI 做的是商用推理效率优化；AwareLiquid 聚焦\textbf{认知连续性与终身学习} —— 在线权重更新 + 跨会话持久状态，而非仅靠上下文内学习。两者同源液态架构，但赛道错位。
\end{itemize}
\end{frame}
