...
\begin{itemize}\itemsep=0.45em
    \item \textbf{电池管理（NASA PCoE 数据集）：} 流式电芯健康度回归，携带状态仅 \textbf{2.6 KB}；在 \textbf{80\% 样本缺失下精度 +7.7\%} —— 对非均匀采样的鲁棒性是液态 ODE 的原生属性，不是补丁。
    \item \textbf{智能穿戴 —— 开源唤醒词（O1-Sound）：} 常开关键词识别，\textbf{5.12 KB 恒定状态}；无云端依赖，MCU 级硬件毫秒响应。
    \item \textbf{金融文档（M2 适配器）：} 文档抽取问答 \textbf{91.7\% 准确率（44/48）}，全程本地私有化。
    \item \textbf{规划中（情景，非承诺）：} 车载座舱系统、端侧个人助手、工业哨兵机器人（DualSpeedSentry 服务栈已容器化）。
\end{itemize}
\vspace{0.3em}
