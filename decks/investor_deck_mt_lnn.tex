...
\begin{frame}{Two Product Lines: Cognitive Slow Thinking + Edge Fast Thinking}
\small
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{M-series (M1/M2) --- Cognitive Slow Thinking}
...
\end{column}
\begin{column}{0.5\textwidth}
\textbf{O-series (O1) --- Edge Fast Thinking}
\begin{itemize}\itemsep=0.3em
    ...
    \item Targets: wearables, automotive, industrial streaming sensors.
\end{itemize}
\end{column}
\end{columns}
\vspace{0.4em}
...
\begin{itemize}\itemsep=0.3em
    ...
    \item \textbf{1232 automated tests permanently green} on CPU --- every claim backed by a script and an artefact.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\end{columns}
\vspace{0.3em}
{\footnotesize\itshape Honest boundary: EWC, experience replay and generative dream-replay are all validated multi-seed at small scale; \textbf{not yet reproduced at $\geq$1B}.}
\end{frame}
...
\begin{frame}{Core Performance 2: Memory That Survives a Dropped Context Window}
\begin{itemize}
...
\end{itemize}
\vspace{0.2em}
\textbf{From the document-QA adapter} (a separate product line --- a retrieval/compression layer around a \emph{frozen} base model, not the liquid architecture):
\begin{itemize}\itemsep=0.4em
    \item \textbf{Financial documents:} \textbf{91.7\% (44/48)} on a labelled multi-domain evaluation set, at \textbf{{\raise.17ex\hbox{$\scriptstyle\sim$}}1.3 model calls and {\raise.17ex\hbox{$\scriptstyle\sim$}}2.8k tokens per question}, fully on-prem. 179 tests green.
\end{itemize}
\vspace{0.2em}
{\footnotesize Pipeline (scenario, not commitment): automotive cabin systems, on-device assistants, industrial sentry robots (DualSpeedSentry serving stack already containerized). The long-term goal is to \emph{merge} the two lines --- replacing the adapter's lexical retrieval with the architecture's native liquid memory.}
\vspace{0.3em}
...
\begin{itemize}\itemsep=0.3em
    \item \textbf{Year 1 (validation):} 5--10 paid design-partner PoCs in regulated B2B (Law / Finance / Defense). Goal: convert $\geq$ 3 to multi-year licenses; publish a 2B-scale checkpoint, a three-size scaling curve, and an open benchmark report.
