...
\begin{itemize}\itemsep=0.45em
    \item \textbf{Battery management (NASA PCoE):} streaming cell state-of-health regression with a \textbf{2.6 KB} carried state; \textbf{+7.7\% accuracy at 80\% missing samples} --- robustness to irregular sampling is native to liquid ODEs, not a patch.
    ...
    \item \textbf{Financial documents (M2 adapter):} \textbf{91.7\% accuracy (44/48)} on document-extraction QA, fully on-prem.
    \item \textbf{Pipeline (scenario, not commitment):} automotive cabin systems, on-device assistants, industrial sentry robots (DualSpeedSentry serving stack already containerized).
\end{itemize}
\vspace{0.3em}
...
On-Device Continual Learning & \textcolor{green}{Yes} & \textcolor{red}{No} & \textcolor{red}{No} & \textcolor{red}{No} & \textcolor{orange}{LoRA only} \\
\bottomrule
\end{tabular}
}
\end{table}
\vspace{0.3em}
{\footnotesize \textbf{vs.\ Liquid AI:} they optimize commercial inference efficiency; AwareLiquid targets \textbf{cognitive continuity and lifelong learning} --- online weight updates and persistent state, not just in-context learning.}
\end{frame}
